...
class TrainerMultiple(nn.Module):
    def __init__(self, hyperparams, train=True):
        ...
        self.channel_reduction = nn.Conv2d(
            in_channels=16,  # Original number of channels
            out_channels=3,  # Reduced number of channels
            kernel_size=1,
            stride=1
        )
        ...
        # Optimizer setup
        self.optimizer = optim.AdamW(self.unet.parameters(), lr=1e-4, weight_decay=1e-5)
    ...
    def train_step(self, images, labels):
        ...
        images = F.interpolate(images, size=(self.config.img_size, self.config.img_size), mode='bilinear', align_corners=False)
