...
from trans_unet import vit_seg_configs_classifier as configs
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        # Use the [CLS] token for classification if available
        if self.config.use_cls_token:
            cls_token = x[:, 0]  # Assuming the first token is the [CLS] token
            x = cls_token
        else:
            # Alternatively, aggregate the token embeddings
    ...
    def load_from(self, weights):
        with torch.no_grad():
            ...
            if posemb.size() == posemb_new.size():
                ...
                if self.config.use_cls_token:
                    # Handle case where class token is present
                    cls_posemb = posemb[:, :1]  # Extract class token
                    posemb_grid = posemb[:, 1:]  # Rest of the embeddings
                else:
                    posemb_grid = posemb
                    cls_posemb = None
                ...
                posemb_grid_resized = ndimage.zoom(posemb_grid, zoom, order=1)  # Resize

                # Flatten the resized grid
                posemb_grid_resized = posemb_grid_resized.reshape(-1, posemb_grid_resized.shape[-1])

                if self.config.use_cls_token:
                    # Ensure class token has the right dimensions
                    if cls_posemb.ndim == 2:
                        cls_posemb = cls_posemb.unsqueeze(0)

                    # Concatenate the class token and the resized grid embeddings
                    posemb_grid_resized = posemb_grid_resized.reshape(1, -1, posemb_grid_resized.shape[-1])

                    # Handle off-by-one errors in token count
                    if posemb_grid_resized.shape[1] == posemb_new.shape[1] - 1:
                        posemb_resized = np.concatenate([cls_posemb, posemb_grid_resized], axis=1)
                    else:
                        raise ValueError(
                            f"Mismatch in token count: pretrained tokens {posemb_grid_resized.shape[1]} and model tokens {posemb_new.shape[1]}")
                else:
                    posemb_resized = posemb_grid_resized.reshape(1, -1, posemb_grid_resized.shape[-1])

                # Copy the resized positional embeddings to the model
                self.transformer.embeddings.position_embeddings.copy_(np2th(posemb_resized))
